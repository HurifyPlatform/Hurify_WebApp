...
class NewPassword extends React.Component{
	...
	resetpassClick(event)
	{
		...
		else if (this.state.user.password != this.state.user.retypenewpass) {
			...
			<div className={resendStyles.formContainer + (ModalStore.modalDisplayed['newpass'] == false ? (' ' + resendStyles.close) : '')}>
				<form action='/' onSubmit={this.forgotClick}>
					...
					<div className={resendStyles.content}>
							...
							<input className={resendStyles.inputField} type="text" placeholder="code" name="forgotPasswordToken" value={this.state.token} onChange={this.changeUser} />
							<input className={resendStyles.inputField} type="password" placeholder="New Password" name="password" value={this.state.password} onChange={this.changeUser} />
							<input className={resendStyles.inputField} type="password" placeholder="Retype New Password" name="retypenewpass" value={this.state.retypenewpass} onChange={this.changeUser} />
							<a className={resendStyles.btn} href="#" onClick = {this.resetpassClick} style={{textDecoration:'none'}}>Send</a>
				 </div>
			  </form>
			</div>
		);
