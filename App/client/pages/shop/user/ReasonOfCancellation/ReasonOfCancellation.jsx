...
class ReasonOfCancellation extends React.Component{
	...
	SubmitCancellation(event)
	{
	    ...
	    orderId:crypt.decrypt(cookies.get('cancelOrderId')),
	...
	render(){
		return(
			<div className={"reason_formContainer" + (ModalStore.modalDisplayed['reason'] == false ? (' ' + "reson_close") : '')}>
				<form action='/' onSubmit={this.SubmitCancellation}>
					...
					<div className="content">
							...
							<input type="submit" className="cancel_btn" id="cancel_btn" disabled={this.state.value1} value="Submit"/>
				 		</div>
			  </form>
